...
- PR タイトル・本文を考える際は、必ず `natural-japanese` スキルを使用する。文章作成のみの依頼にも適用する。
- 不明な点はユーザーに確認する。
- 機密情報は含めない。
...
5) PR タイトルと本文を作成する
   - 作成前に `../natural-japanese/SKILL.md` を読み、同スキルの手順に従って執筆・検査・推敲を行う。
   - テンプレートと既存 PR の形式に合わせる。
   - 変更内容・影響範囲・確認手順を簡潔に記載する。
   - タイトルは主な変更が一読で伝わる表現にし、本文は変更の目的と内容がわかる自然な日本語にする。
   - 推敲では、テンプレートの見出し・必須項目、技術的な識別子、リンクを維持する。差分にない変更や未実施の検証結果を補わない。
6) 実行モードを分岐する
